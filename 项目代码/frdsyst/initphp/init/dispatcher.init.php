...
class dispatcherInit {
	...
	private function parseRewriteUri($request) {
		...
		// BUGFIX:HEWEI 20150323 新版initphp默认只有在请求参数m,c,a都为空，则走默认的
		if ($InitPHP_conf['ismodule'] == true && !isset($_GET['m'])) $_GET['m'] = $InitPHP_conf['controller'] ['default_module'];
		if (!isset($_GET['c'])) $_GET['c'] = $InitPHP_conf['controller'] ['default_controller'];
		if (!isset($_GET['a'])) $_GET['a'] = $InitPHP_conf['controller'] ['default_action'];
		return $request;
	}
