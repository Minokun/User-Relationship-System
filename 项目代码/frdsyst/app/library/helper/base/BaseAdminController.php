			...
			if($this->getA()!='login'&&$this->getA()!='loginSubmit'){
				...
				if($this->getA()=='loginout'){
					$tool->save("管理员退出", array(
							"user"=>$_SESSION["manage_user"],
							"data"=>array("newData"=>"","oldData"=>"")
					));
					if(isset($_SESSION["manage_user"])){
						unset($_SESSION["manage_user"]);
					}
					if(isset($_SESSION["manage_user_group"])){
						unset($_SESSION["manage_user_group"]);
					}
					$message['status']="退出成功";
					$message['url']=BASE_PATH."admin/index/login";
					$message['infor']="返回首页";
				...
				//登录判断
				if($this->isLogin()){
					$usergroup = $_SESSION["manage_user_group"];
					$a = $this->getA();
					$c = $this->getC();
...
// 					}
					...
					$this->view->display("admin/index/login");
					exit();
				}
			}
			if($this->getA()=="login"){
				if($this->isLogin()){
					$this->view->display("admin/index/index");
					exit();
				}
				$this->view->display("admin/index/login");
			}
		...
		public function isLogin(){
			...
			if($this->getA()=="uploadHeadPic"){//文章图片上传不做验证
				return true;
			}
			if($this->getA()=="uploadInfoPic"){//文章图片上传不做验证
				return true;
			}
			if(isset($_SESSION["manage_user"])){
				return true;
